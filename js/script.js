...
const chackebox = document.querySelector('.checkbox__keys')
const switcher = document.querySelector('.switcher')
const keySelect = document.querySelector('.disable-keys')


const playNote = (nota) => {
    const audio = new Audio(`../notes/notes_${nota}.wav`)
    audio.play()
}
...
chackebox.addEventListener('change', ( { target } ) => {
    if(target.checked) {
        switcher.classList.add('switcher--active')
        keySelect.classList.remove('disable-keys')
        return
    }  
    switcher.classList.remove('switcher--active')
    keySelect.classList.add('disable-keys')
